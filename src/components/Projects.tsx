...
export const Projects: React.FC = () => {
  ...
  const renderItem = (project: Project) => (
    <div
      key={project.id}
      className="flex flex-col shadow-xl rounded-lg bg-white h-full p-6 pt-2"
    >
      <div className="flex justify-between items-center py-3 pr-1">
        <div>
          <a
            href={project.link ?? undefined}
            target="_blank"
            rel="noopener noreferrer"
            className={`inline-flex items-center gap-1.5 font-bold text-xl ${
              project.link ? "hover:underline hover:text-blue-500" : ""
            }`}
          >
            {project.title}
            {project.live && (
